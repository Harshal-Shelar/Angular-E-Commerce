...
export class ProductService {
  ...
  popularProducts() {
    return this.http.get<product[]>('http://localhost:5000/products?_limit=3');
  }
  ...
  getCartList(userId: number) {
    return this.http
      .get<product[]>('http://localhost:5000/cart?userId=' + userId, {
        observe: 'response',
      })
      .subscribe((result) => {
  ...
  deleteCartItems(cartId: number) {
    return this.http.delete('http://localhost:5000/cart/' + cartId).subscribe((result) => {
      this.cartData.emit([]);
    })
  }
